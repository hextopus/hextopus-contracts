...
async function increaseTime(provider, seconds) {
    await send(provider, "evm_increaseTime", [seconds] as any)
}
...
function encodeParams(inputs) {
    ...
    try {
        parameters = iface.encode(types, values).replace(/^(0x)/, '');
    } catch (ex) {
        console.log(ex);
    }
...
export { encodeParams, deployContract, expandDecimals, bigNumberify, getPriceBits, getExpandedPrice, getBlockTime, toUsd, mineBlock, increaseTime, priceUpdateAndExcute, encode}